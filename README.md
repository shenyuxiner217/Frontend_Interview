...
* HTTP协议
  ...
  是超文本传输协议，定义了客户端与服务器之间交换数据的方式，使用TCP作为传输层协议，保证了数据传输的可靠性。
...
* XSS
  跨站脚本攻击，通过代码注入恶意脚本进行攻击，使之在用户的浏览器上运行从而盗取用户的信息。
  ...
  防御：
  1. 对需要插入到 HTML 中的代码做好充分的转义及特殊字符的过滤；
  2. 充分利用CSP内容安全策略，建立一个白名单，告诉浏览器哪些外部资源可以加载和执行： 禁止向第三方提交数据；限制加载其他域下的资源；禁止执行内连脚本；
